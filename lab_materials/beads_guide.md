...
4. **Сделайте commit изменений проекта** (документация/код).
5. **Закройте задачу** после завершения.
6. **Выполните `bd sync`**, если синхронизация не прошла автоматически.
...
`.beads/issues.jsonl`, поэтому отдельный `git commit`/`git push` этих

...
5. **ENV PORT=8080** — значение по умолчанию. В Yandex Cloud эта переменная
   задаётся автоматически платформой Serverless Containers (или вручную
   в параметрах ревизии).
...
**Откуда берётся `$PORT`:**
- в Serverless Containers это переменная окружения, которую платформа
  устанавливает при запуске контейнера;
- в локальном запуске можно оставить значение по умолчанию (`ENV PORT=8080`)
  или переопределить его через `-e PORT=...` при `docker run`.

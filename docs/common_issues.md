...
**Причина:** frontend и backend работают на разных портах (разные origin),
а сервер не разрешил такие запросы. Для `POST` с JSON браузер также выполняет
предварительный `OPTIONS`‑запрос (preflight).

**Решение:**
- включить CORS на backend (Rocket) и разрешить `OPTIONS`, если это
  предусмотрено заданием;
- использовать прокси Trunk для локальной разработки.

Подробный учебный разбор и варианты решений: `lab_materials/lab_work.md`.
...
## 9. Проблемы с beads (issue‑трекер проекта)

### Ошибка: "LEGACY DATABASE DETECTED"

**Причина:** база beads была создана в старой версии и не содержит

...
4. **Синхронизация с git:** все коммиты выполняются при активных хуках.
5. **Закрытие задачи:** `bd close <id>` выполняется только после коммита,
   успешной синхронизации и push.

**Важно:** `bd sync` сам делает commit и push. После `bd sync` отдельный

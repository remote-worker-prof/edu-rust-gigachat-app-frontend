...
По умолчанию `bd sync` выполняет `git pull`, поэтому отдельный `git pull`
до или после `bd sync` не нужен.
...
Если вы отредактировали, например, `src/app.rs`, но не сделали `git add -A`

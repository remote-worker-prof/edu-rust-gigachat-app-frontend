...
4. **Коммит изменений проекта:** зафиксировать изменения в документации/коде.
5. **Закрытие задачи:** `bd close <id>` выполняется после коммита проекта.
6. **Синхронизация beads:** `bd sync` выполняется после `bd close`.

**Важно:** `bd sync` сам делает commit и push для `.beads/issues.jsonl`.
Эти изменения **не** коммятся вручную. После `bd sync` отдельный
`git push` обычно не нужен. Повторный push допускается только если
`bd sync` не выполнялся или завершился с ошибкой.
...
- закрывать задачу до коммита изменений проекта и `bd sync`;
